...
1. **DTR replication** — reproduces the Deep-Thinking-Ratio (DTR) sample-selection
   method from *"Think Deep, Not Just Long"*, plus the confidence-signal trajectory
   analysis (Self-Certainty, log-probability). *(Thesis appendices A & B.)*
2. **Intervention framework** — a pluggable engine that steers a reasoning model's
...
Model used throughout: **Qwen3-4B-Thinking-2507**.
...
## Part 1 — DTR replication & confidence analysis
...
```bash
python tests/test_strategy_renames_smoke.py
python tests/test_think_budget_equivalence.py
```

## Notes

- `outputs/` holds generated experiment artifacts — large, kept out of git.
