...
-------------
  ...
  selfcert Negative entropy of final-layer distribution (needs HF, final layer only)
...
def main():
    ...
    parser.add_argument("--output-dir", default="outputs/results_verbose_substring_occur")
    parser.add_argument(
        "--ranking-mode",
        default="all",
    ...
    for problem, samples in zip(problems, all_samples):
        answers = [_normalize_for_comparison(normalize_numeric_answer(extract_answer(s.generated_text, problem.answer_type))) for s in samples]
    ...
    needs_model = any(m in ("dtr", "selfcert") for m in modes_to_run)
                ...
                "sample_extractions": [
                    {
                        ...
                        # per-sample scores for every mode that was run
                        **{
